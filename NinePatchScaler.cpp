...
wxBitmap NinePatchScaler::GetScaledBitmap(const wxString& resourceName, const wxSize& baseSize, const wxSize& targetSize, int borderSize)
{
    ...
    // Load original bitmap from memory filesystem
    wxBitmap originalBitmap;
    wxFileSystem fs;
    wxString memoryPath = wxString::Format("memory:%s", resourceName);
    wxFSFile* file = fs.OpenFile(memoryPath);
...
wxBitmap NinePatchScaler::performNinePatchScale(const wxBitmap& source, const wxSize& targetSize, int borderSize) const
{
    ...
    for (int y = 0; y < borderSize; y++)
    {
        ...
        {
            targetImage.SetRGB(x, y, sourceImage.GetRed(x, y), sourceImage.GetGreen(x, y), sourceImage.GetBlue(x, y));
        }
    }
    ...
    for (int y = 0; y < borderSize; y++)
    {
        for (int x = 0; x < borderSize; x++)
        {
            targetImage.SetRGB(targetWidth - borderSize + x, y, sourceImage.GetRed(srcWidth - borderSize + x, y), sourceImage.GetGreen(srcWidth - borderSize + x, y),
    ...
    for (int y = 0; y < borderSize; y++)
    {
        for (int x = 0; x < borderSize; x++)
        {
            targetImage.SetRGB(x, targetHeight - borderSize + y, sourceImage.GetRed(x, srcHeight - borderSize + y), sourceImage.GetGreen(x, srcHeight - borderSize + y),
    ...
    for (int y = 0; y < borderSize; y++)
    {
        for (int x = 0; x < borderSize; x++)
        {
            targetImage.SetRGB(targetWidth - borderSize + x, targetHeight - borderSize + y, sourceImage.GetRed(srcWidth - borderSize + x, srcHeight - borderSize + y),
                               sourceImage.GetGreen(srcWidth - borderSize + x, srcHeight - borderSize + y), sourceImage.GetBlue(srcWidth - borderSize + x, srcHeight - borderSize + y));
        }
    }
